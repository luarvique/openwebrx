...
ARCH=$(uname -m)
#IMAGES="openwebrx-full openwebrx"
ALL_ARCHS="x86_64"
TAG=${TAG:-"luarvique"}
ARCHTAG="${TAG}-${ARCH}"
...
build () {
  ...
  podman build --pull -t openwebrx-base:${ARCHTAG} -f docker/Dockerfiles/Dockerfile-base .
  podman build --build-arg ARCHTAG=${ARCHTAG} -t openwebrx:${ARCHTAG} -f docker/Dockerfiles/Dockerfile-ni .
  podman rmi openwebrx-base:${ARCHTAG} --force .
